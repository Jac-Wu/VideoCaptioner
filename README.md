...
Whisper 版本有 WhisperCpp、PyWhisperCpp（macOS 专用）和 fasterWhisper（推荐） 三种，都需要自行在软件内下载模型。

**PyWhisperCpp 特性**（仅 macOS）：
- ✅ **CoreML 硬件加速**：利用 Apple 芯片的神经引擎，转录速度更快
- ✅ **Silero VAD 语音检测**：自动过滤静音片段，减少幻觉，提升准确率
  - 🆕 **语音填充**：在检测到的语音前后添加缓冲时长（默认 2000ms），防止切掉词首词尾
  - 🆕 **智能静音合并**：只在长时间静音（默认 1000ms）时分割，保持自然停顿的完整性
  - 🆕 **上下文拼接**：使用前一段文本作为下一段的上下文提示，减少断层丢字
- ✅ **多进程并发处理**：支持同时处理多个音频段，大幅提升长视频转录速度
- ✅ **可配置并发数**：根据系统资源灵活调整（1-8 个并发任务）
  - ⚠️ 注意：启用上下文拼接后将禁用并行处理以保证上下文连贯性
...
- **macOS 用户可使用 PyWhisperCpp，支持 CoreML 加速和多进程并发**
...
- `VAD过滤`：开启后，VAD（语音活动检测）将过滤无人声的语音片段，从而减少幻觉现象。建议保持默认开启状态。
  - **PyWhisperCpp 特有**：支持 Silero VAD 高精度语音检测，并提供以下可配置参数：
    - `语音填充时长`：在检测到的语音前后添加缓冲（0-5000ms，默认 2000ms），防止切掉词首词尾
    - `最小静音时长`：分割语音段的最小静音时长（100-5000ms，默认 1000ms），避免在自然停顿处切断
    - `上下文拼接`：使用前一段文本作为下一段的上下文提示，提升转录连贯性（启用后禁用并行处理）
  - **并发处理**：PyWhisperCpp 支持多进程并发转录（可配置 1-8 个并发任务），大幅提升长视频处理速度
  - 💡 **使用建议**：如果不懂，VAD 选项建议直接保持默认即可
